...
router.post("/webhook", async (req, res) => {
  ...
  try {
    if (type === "payment") {
      // Aquí deberías consultar el estado del pago usando el ID (data.id) con el SDK
      // Por simplicidad en este paso, asumimos que si llega aquí y es 'approved' actualizamos.
      // En producción, verifica el estado real del pago consultando a la API de MP.
      
      // Nota: MercadoPago envía el ID del pago, necesitas consultarlo para ver 'external_reference' (userId)
      // y el estado 'status'.
      
      // Implementación simplificada:
      logger.info("Webhook MercadoPago recibido", req.body);
      
      // TODO: Implementar verificación de pago real con paymentId = data.id
      // const payment = await client.payment.get({ id: data.id });
      // if (payment.status === 'approved') {
      //    const userId = payment.external_reference;
      //    await User.findByIdAndUpdate(userId, { planLevel: "premium", subscriptionId: `mp_${data.id}` });
      // }
    }
