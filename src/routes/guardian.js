...
router.post("/learn", authenticate, async (req, res) => {
  ...
  try {
    ...
    // 2.5 Calcular Fricción y ADN Léxico
    const friction = GuardianService.calculateFriction(originalText, editedText);
    const newLexicon = GuardianService.extractLexicalDNA(originalText, editedText);
    
    // Mezclar léxico nuevo con existente (mantener últimos 50)
    const currentLexicon = contact.guardianMetadata?.preferredLexicon || [];
    const updatedLexicon = [...new Set([...currentLexicon, ...newLexicon])].slice(-50);

    // 3. Actualizar el Perfil del Contacto
    contact.guardianMetadata = {
      lastUserStyle: userStyle,
      preferredLexicon: updatedLexicon,
      editFrictionHistory: [...(contact.guardianMetadata?.editFrictionHistory || []), friction].slice(-10),
      trained: true,
    };
    ...
    contact.relationalHealth = Math.min(
      10,
      Math.max(1, contact.relationalHealth + editedScore),
    );
